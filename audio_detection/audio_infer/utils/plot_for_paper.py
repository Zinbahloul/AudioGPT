...
def plot_classwise_iteration_map(args):
    ...
    for col in range(0, 3):
        ...
        for _ix in ranges[col]:
            _label = f'{crop_label(config.labels[sorted_indexes[_ix]])} ({add_comma(config.full_samples_per_class[sorted_indexes[_ix]])})'
...
def plot_six_figures(args):
    ...
    plt.savefig(save_out_path)
    print(f'Save figure to {save_out_path}')
...
def plot_long_fig(args):
    ...
    ax1b.scatter(
        np.arange(N)[sorted_label_quality is None],
        0.5 * np.ones(len(np.arange(N)[sorted_label_quality is None])),
        s=12,
        c='r',
        linewidth=0.8,
        marker='_',
